	...
	<form>
		<div class="form-row align-items-center mb-3">
			<div class="col-12 col-md-2 mb-2">
				<a class="btn btn-outline-primary" href="./?modulo=contato&acao=cadastrar"><i class="fa fa-plus"></i> Adicionar contato</a>
			</div>
					...
					<tbody>
						...
						while($contato = $contatos->fetch(PDO::FETCH_ASSOC)){
							...
							<tr>
								...
								<td><?=date_format(date_create($contato['datacadastro']), 'd/m/Y')?></td>
								<td><?=$contato['nome']?></td>
								<td><?=$contato['email']?></td>
								<td><?=$contato['telefone']?></td>
								<td><?=$contato['origem']?></td>
								<td><?=($contato['newsletter'] == 1) ? 'Sim' : 'Não'?></td>
								...
								<td><a href="#" class="email"  data-nome="<?=$contato['nome']?>" data-contato="<?=$contato['id']?>" data-destinatario="<?=$contato['email']?>" data-email="<?=$contato['email']?>"><i class="fa text-primary fa-2x fa-envelope"></i></a> </td>
							</tr>
							<?php
						}
						?>
